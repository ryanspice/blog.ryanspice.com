...
type ArticleShareImage = Pick<ArticleMeta, 'locale' | 'slug' | 'title' | 'updatedDate'>;
...
export function articleShareImagePath(article: ArticleShareImage, site: ShareImageSite): string {
	...
	return `/img/social/${site.id}/articles/${localizedSlug}.png?v=${encodeURIComponent(article.updatedDate)}`;
}
