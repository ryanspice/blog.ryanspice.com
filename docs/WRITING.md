...
3. Visual pass: make the side/focal slot intentional. For new public articles, generate a Codex raster focal visual unless a licensed/source image is clearly better; save the optimized asset under `static/img/articles/<slug>/`; set `image`, alt text, credit, and position; set `row_image` for cards when useful; and reserve inline Markdown images for diagrams/screenshots inside the body. Avoid showing the same image in the hero side slot and again as the first body figure. When `further_reading` links point to internal articles with focal art, the rendered cards should keep that art faded in the background for style without sacrificing readability.
